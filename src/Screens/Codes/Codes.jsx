...
import { ArrowDropUpIcon } from "../../components";
...
import {
  patientSummary,
  patientTabFlag,
  rejectScanCodeRequest,
} from "../../redux/userSlice/patientInfoSlice";
import {
  existingRejectInfo,
  existingValue,
  recaptureValue,
...
export const Codes = () => {
  ...
  useEffect(() => {
    const processEventData = async () => {
      const itemsToProcess = eventData.filter(
        (item) =>
          !newEventData.some((existingItem) => existingItem.id === item.id)
      );
  ...
  const handleSubmitRedirect = async (tabs) => {
    ...
    const isSummaryModal =
      tabs["patient_dashboard_summary_screen"]?.active || false;
    if (isSummaryModal) {
  ...
  const handleSubmit = async () => {
    ...
    if (existingCodeReject.length > 0) {
      ...
      requestBody = requestBody.delete_codes
        ? {
            ...requestBody,
            delete_codes: { ...requestBody.delete_codes, existing_codes },
          }
    ...
    if (recaptureCodeReject?.length > 0) {
      ...
      requestBody = requestBody.delete_codes
        ? {
            ...requestBody,
            delete_codes: { ...requestBody.delete_codes, recapture_codes },
          }
    ...
    if (duplicateCodeReject?.length > 0) {
      ...
      requestBody = requestBody.delete_codes
        ? {
            ...requestBody,
            delete_codes: { ...requestBody.delete_codes, duplicate_codes },
          }
    ...
    if (suspectCodeReject?.length > 0) {
      ...
      requestBody = requestBody.delete_codes
        ? {
            ...requestBody,
            delete_codes: { ...requestBody.delete_codes, new_codes },
          }
    ...
    try {
      ...
      if (result?.meta?.requestStatus === "fulfilled") {
        ...
        const isAthenaModal = tabs["type"]?.value === "Athena";
        const exampleMetadata = {
          event_type: isAthenaModal
            ? "SUMMARY_ATHENA_MODAL_SUBMIT_AND_CLOSE"
            : "SUMMARY_EPIC_MODAL_SUBMIT_AND_CLOSE",
          metadata: {
            identifier: tabs?.["user"]?.value || "",
            provider_name: doctorDetail?.doctor_name || "",
            patient_id: user?.data?.userInfo?.mrn || "",
  ...
  useEffect(() => {
    if (slug && tabData && user?.data?.userInfo?.mrn) {
      // Add check for user data
  ...
  // const localData =
  const existingConditionNew = useSelector(
    (state) => state.user.data.existingCondition
  );
  const duplicateCodeNew = useSelector(
    (state) => state.user.data.duplicateCode
  );
  const recaptureCodeNew = useSelector(
    (state) => state.user.data.recaptureCode
  );
  ...
  if (tabs && tabs["patient_dashboard_extended_data"]?.active) {
    codesData = [
      ...
      },
      {
        ...
        container: (
          <Scans
            setRejectScanCode={setRejectScanCode}
            rejectScanCode={rejectScanCode}
            sessionObject={sessionObject}
            handleAddEventData={handleAddEventData}
          />
        ),
        isShow:
          (tabs && tabs["patient_dashboard_extended_da"]?.active) || false,
      },
      {
        key: 6,
  ...
  const handleDelete = (event, item, key) => {
    ...
    if (key === "existing") {
      ...
      setRejectScanCode(
        (prev) => [...prev.filter((items) => items.id !== item.id)] // Replace 'someId' with the id to match
      );
    }
    localStorage.setItem(
      `sessionObject_${userDetail.mrn}`,
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              {!loadingSummary ? (
                                summary?.recapture_codes_count +
                                  summary?.suspect_conditions_count +
                        ...
                        <div
                          style={styles}
                          className="offcanvas offcanvas-top position-absolute"
                          tabindex="-1"
                          id="offcanvasTop"
                        ...
                        >
                              ...
                              <Grid container>
                                <Grid
                                  ...
                                  sx={{
                                    ...
                                    gap: "1rem", // Adjust gap between button and text
                                  }}
                                >
                                  <PrimaryButton
                                    onClick={(event) => respExpand(event, 1)}
                                    disabled={
                                      summary?.existing_codes_count ===
                                        undefined ||
                                      summary?.existing_codes_count === 0
                                    }
                                    sx={{
                                      ...
                                      ":hover": {
                                        backgroundColor:
                                          theme.palette.error.A200,
                                      },
                                      ":disabled": {
                                        backgroundColor:
                                          theme.palette.error.A200, // Same as normal state
                                  ...
                                  >
                                    ...
                                    urgent existing conditions requiring
                                    recapturing.
                                  </Typography>
                                </Grid>
                              </Grid>
                              <Grid container sx={{ my: 2 }}>
                                <Grid
                                  ...
                                  sx={{
                                    display: "flex",
                                    alignItems: "center", // Align items vertically
                                    gap: "1rem", // 1rem gap between button and text
                                  }}
                                >
                                  <PrimaryButton
                              ...
                              <Grid container sx={{ my: 2 }}>
                                ...
                                >
                                  ...
                                  >
                                    ...
                                    existing conditions that are not in the
                                    problem list.
                                  </Typography>
                                </Grid>
                              </Grid>
                          ...
                          <div style={backdropStyle} class="backdrop-filter">
                            {" "}
                          </div>
                        </div>
                        ...
                        >
                          <CardContent>
                            ...
                            <Box>
                              ...
                              {tabs &&
                                tabs["patient_dashboard_extended_data"]
                                  ?.active && (
                                  <Grid container sx={{ pb: 2, mb: 0 }}>
                                    <Grid item lg={9} md={9} sm={10} xs={10}>
                                      <Typography className="HeadSummary">
              ...
              {!isModalSubmit && (
                ...
                >
                  ...
                  <CardContent sx={{ position: "relative" }}>
                    ...
                    <div
                      style={{
                        filter: loadingSummary ? "blur(4px)" : "none",
                        pointerEvents: loadingSummary ? "none" : "auto",
                      }}
                    >
                      <Grid
                        container
                      ...
                      >
                        <Grid item lg={2} md={2} sm={2} xs={12}>
                          <PrimaryButton
                            onClick={() =>
                              setExpanded(expanded === 1 ? false : 1)
                            }
                            disabled={
                              summary?.existing_codes_count === undefined ||
                              summary?.existing_codes_count === 0 ||
                              0
                            }
                            sx={{
                      ...
                      >
                        <Grid item lg={2} md={2} sm={2} xs={12}>
                          <PrimaryButton
                            disabled={
                              summary?.suspect_conditions_count === undefined ||
                              summary?.suspect_conditions_count === 0 ||
                              0
                            }
                            sx={{
                      ...
                      <Grid container>
                        <Grid item lg={2} md={2} sm={2} xs={12}>
                          <PrimaryButton
                            disabled={
                              summary?.recapture_codes_count === undefined ||
                              summary?.recapture_codes_count === 0 ||
                              0
                            }
                            onClick={() =>
                              setExpanded(expanded === 3 ? false : 3)
                            }
                            sx={{
                              width: "2.375rem",
                              height: "1.5625rem",
                  ...
                  <CardContent sx={{ position: "relative" }}>
                    ...
                    >
                      <Box>
                            ...
                            <>
                              ...
                              {suspectCodeReject &&
                                ...
                                suspectCodeReject?.map((item, index) => (
                                  ...
                                  >
                                    <Tooltip
                                      title={item[Object.keys(item)].value}
                                    >
                                      <Typography
                                        sx={{
                                          padding: "0px !important",
                        ...
                        <Grid
                          container
                          sx={{ pb: 2, mb: 0, position: "relative" }}
                        >
                          <Grid item lg={9} md={9} sm={10} xs={12}>
                            <Typography className="HeadSummary">
                              Additional diagnoses
                        ...
                        {tabs &&
                          tabs["patient_dashboard_extended_data"]?.active && (
                            <Grid
                              container
                              sx={{
                                borderBottom: "1px solid #00000029",
                                          ...
                                          >
                                            <StylePop className="ChipSpan rejected">
                                              {windowSize.width > 967
                                                ...
                                                ? item?.value?.slice(0, 23) +
                                                  (item?.value?.length > 23
                                                    ? "..."
                                                    : "")
          ...
          <Box>
            ...
            {tabs && tabs["type"]?.value == "Athena" ? (
              ...
              <>
                ...
                >
                  Review actions that have been taken{" "}
                  {!hoagmodalarrow ? (
                              ...
                              >
                                ...
                                >
                                  ...
                                  >
                                    <Grid item lg={12} md={12} sm={12} xs={12}>
                                      <StyledCodeTypography className="">
                                        Codes/Conditions to be actioned in
                                        Reconcile Outside Information tab
                                  ...
                                  >
                                    <Grid item lg={12} md={12} sm={12} xs={12}>
                                      <StyledCodeTypography className="">
                                        Rejected codes/conditions from
                                        Doctustech{" "}
                                      </StyledCodeTypography>
                                    </Grid>
